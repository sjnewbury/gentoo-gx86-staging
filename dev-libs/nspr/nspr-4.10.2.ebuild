...
multilib_src_install() {
	...
	if multilib_is_native_abi ; then
		dobin "${BUILD_DIR}"/config/nspr-config || die "failed to install nspr-config"
	else
		newbin "${BUILD_DIR}"/config/nspr-config nspr-config.${ABI} || die "failed to install nspr-config.${ABI}"
	fi
}
