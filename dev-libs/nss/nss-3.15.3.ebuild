...
# $Header: /var/cvsroot/gentoo-x86/dev-libs/nss/nss-3.15.3.ebuild,v 1.6 2013/12/01 18:10:10 ago Exp $
...
src_prepare() {
	...
	multilib_parallel_foreach_abi nss_multilib_prepare
}
...
multilib_src_install_all() {
	local _abi
	for _abi in $(multilib_get_enabled_abis) ; do
		cat "${T}"/90nss.${_abi} >> ${T}/90nss
	done
	doenvd "${T}/90nss"
}

nss_multilib_postinst() {
...
pkg_postinst() {
	...
	multilib_parallel_foreach_abi nss_multilib_postinst
}
...
pkg_postrm() {
	multilib_parallel_foreach_abi nss_multilib_postrm
}
