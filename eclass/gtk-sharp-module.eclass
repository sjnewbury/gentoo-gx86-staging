...
inherit eutils mono multilib libtool autotools base versionator multilib-build
...
case ${GTK_SHARP_MODULE} in
	...
	art|gnome|gnomevfs|gconf)
		TARBALL="gnome-sharp"
		add_depend "=dev-dotnet/gtk-sharp-${GTK_SHARP_REQUIRED_VERSION}*[${MULTILIB_USEDEP}]"
		has "${GTK_SHARP_MODULE}" "${gapi_users_list}" && \
			add_bdepend "=dev-dotnet/gtk-sharp-gapi-${GTK_SHARP_REQUIRED_VERSION}*[${MULTILIB_USEDEP}]"
		case ${PVR} in
			2.24.1*)
				...
				;;
			2.24.2*)
				add_depend ">=dev-lang/mono-2.7[${MULTILIB_USEDEP}]"
				;;
		esac
		;;
	gnome-desktop|gnome-print|gnome-panel|gtkhtml|gtksourceview|nautilusburn|rsvg|vte|wnck)
		...
		add_depend "=dev-dotnet/gtk-sharp-${GTK_SHARP_REQUIRED_VERSION}*[${MULTILIB_USEDEP}]"
		add_depend "=dev-dotnet/gnome-sharp-2.24*[${MULTILIB_USEDEP}]"
		add_depend "gnome-base/gnome-desktop:2[${MULTILIB_USEDEP}]"
		add_bdepend "=dev-dotnet/gtk-sharp-gapi-${GTK_SHARP_REQUIRED_VERSION}*[${MULTILIB_USEDEP}]"
		;;
	*)
...
case ${PF} in
	...
	gtk-sharp-gapi*)
		...
		;;
	gtk-sharp-*)
		add_bdepend "~dev-dotnet/gtk-sharp-gapi-${PV}[${MULTILIB_USEDEP}]"
		add_depend "~dev-dotnet/glib-sharp-${PV}[${MULTILIB_USEDEP}]"
		add_depend "~dev-dotnet/atk-sharp-${PV}[${MULTILIB_USEDEP}]"
		add_depend "~dev-dotnet/gdk-sharp-${PV}[${MULTILIB_USEDEP}]"
		add_depend "~dev-dotnet/pango-sharp-${PV}[${MULTILIB_USEDEP}]"
		;;
	gdk-sharp-*)
		add_bdepend "~dev-dotnet/gtk-sharp-gapi-${PV}[${MULTILIB_USEDEP}]"
		add_depend "~dev-dotnet/glib-sharp-${PV}[${MULTILIB_USEDEP}]"
	...
	glib-sharp-*)
		add_rdepend "!<=dev-dotnet/gtk-sharp-2.12.7:2"
		add_depend "dev-libs/glib:2[${MULTILIB_USEDEP}]"
		;;
	pango-sharp-*)
		add_bdepend "~dev-dotnet/gtk-sharp-gapi-${PV}[${MULTILIB_USEDEP}]"
		add_depend "~dev-dotnet/glib-sharp-${PV}[${MULTILIB_USEDEP}]"
	...
	gtk-dotnet-*)
		...
		add_depend "~dev-dotnet/gdk-sharp-${PV}[${MULTILIB_USEDEP}]"
		add_depend "~dev-dotnet/pango-sharp-${PV}[${MULTILIB_USEDEP}]"
		add_depend "~dev-dotnet/gtk-sharp-${PV}[${MULTILIB_USEDEP}]"
		add_depend "dev-lang/mono[-minimal,${MULTILIB_USEDEP}]"
		add_rdepend "!<=dev-dotnet/gtk-sharp-2.12.7:2"
		;;
	glade-sharp-*)
		add_bdepend "~dev-dotnet/gtk-sharp-gapi-${PV}[${MULTILIB_USEDEP}]"
	...
	gnome-sharp-*)
		...
		add_depend "~dev-dotnet/gnomevfs-sharp-${PV}[${MULTILIB_USEDEP}]"
		add_depend "~dev-dotnet/art-sharp-${PV}[${MULTILIB_USEDEP}]"
	...
	gconf-sharp-*)
		...
		add_depend "=dev-dotnet/glade-sharp-${GTK_SHARP_REQUIRED_VERSION}*[${MULTILIB_USEDEP}]"
		add_depend "~dev-dotnet/gnome-sharp-${PV}[${MULTILIB_USEDEP}]"
		add_depend "~dev-dotnet/art-sharp-${PV}[${MULTILIB_USEDEP}]"
		;;
	gnomevfs-sharp-*)
...
phase_hook() {
	...
	# Strip leading underscore if from multilib wrapper
	local calling_function
	if [[ "${FUNCNAME[1]}" =~ ^_.*$ ]]; then
		calling_function=${FUNCNAME[1]:1}
	else
		calling_function=${FUNCNAME[1]}
	fi
	if [[ "$(type -t ${1}${calling_function#gtk-sharp-module})" = "function" ]]
	then
		ebegin "Phase-hook: Running ${1}${calling_function#gtk-sharp-module}"
		${1}${calling_function#gtk-sharp-module}
		eend 0
	fi
}
...
gtk-sharp-tarball-post_src_prepare() {
	...
	cd "${BUILD_DIR}/${GTK_SHARP_MODULE_DIR}"
	sed -i \
...
# If set, EAUTORECONF will be run during src_prepare.
	...
	multilib_parallel_foreach_abi _gtk-sharp-module_src_prepare
}
...
# Is exported.
_gtk-sharp-module_src_configure() {
...
gtk-sharp-module_src_configure() {
	multilib_parallel_foreach_abi _gtk-sharp-module_src_configure
}
...
# Is exported.
_gtk-sharp-module_src_compile() {
	rm -f "${BUILD_DIR}"/Makefile* &> /dev/null
	pushd "${BUILD_DIR}/${GTK_SHARP_MODULE_DIR}" &>/dev/null
	emake CSC=/usr/bin/gmcs || die "emake failed"
	popd &>/dev/null
}
gtk-sharp-module_src_compile() {
	multilib_parallel_foreach_abi _gtk-sharp-module_src_compile
}
