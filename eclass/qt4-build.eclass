...
qt4-build_pkg_setup() {
	...
	PATH="${BUILD_DIR}/bin${PATH:+:}${PATH}"
	if [[ ${CHOST} != *-darwin* ]]; then
		LD_LIBRARY_PATH="${BUILD_DIR}/lib${LD_LIBRARY_PATH:+:}${LD_LIBRARY_PATH}"
	else
		DYLD_LIBRARY_PATH="${BUILD_DIR}/lib${DYLD_LIBRARY_PATH:+:}${DYLD_LIBRARY_PATH}"
...
prepare_directories() {
	for x in "$@"; do
		pushd "${BUILD_DIR}"/${x} >/dev/null || die
...
symlink_binaries_to_buildtree() {
	for bin in qmake moc uic rcc; do
		ln -s "${QTBINDIR}"/${bin} "${BUILD_DIR}"/bin/ || die "symlinking ${bin} to ${BUILD_DIR}/bin failed"
	done
}
...
fix_library_files() {
	...
	for libfile in "${D}"/${QTLIBDIR}/{*.la,*.prl,pkgconfig/*.pc}; do
		if [[ -e ${libfile} ]]; then
			sed -i -e "s:${BUILD_DIR}/lib:${QTLIBDIR}:g" ${libfile} || die "sed on ${libfile} failed"
		fi
	done
	...
	for libfile in "${D}"/${QTLIBDIR}/pkgconfig/*.pc; do
		if [[ -e ${libfile} ]]; then
			sed -i -e "s:${BUILD_DIR}/bin:${QTBINDIR}:g" ${libfile} || die "sed on ${libfile} failed"
