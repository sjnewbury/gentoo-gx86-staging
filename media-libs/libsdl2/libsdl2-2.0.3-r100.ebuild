...
IUSE="3dnow alsa altivec +audio custom-cflags dbus directfb fusionsound gles haptic +joystick mmx nas opengl oss pulseaudio sse sse2 static-libs +threads tslib udev +video X xinerama xscreensaver wayland"
REQUIRED_USE="
	alsa? ( audio )
	fusionsound? ( audio )
	gles? ( video )
...
RDEPEND="
	...
	directfb? ( >=dev-libs/DirectFB-1.0.0[${MULTILIB_USEDEP}] )
	fusionsound? ( >=media-libs/FusionSound-1.1.1[${MULTILIB_USEDEP}]  )
...
src_prepare() {
	...
	epatch "${FILESDIR}"/${PN}-2.0.2-static-libs.patch
...
#		--includedir="/usr/$(get_libdir)/${PN}/include" \
	...
	econf \
		...
		$(use_enable wayland wayland-shared) \
		--disable-video-cocoa \
		${directfbconf} \
...
multlib_src_install() {
	...
	if ! multilib_is_native_abi ; then
		mv "${ED}"/usr/bin/sdl2-config "${ED}"/usr/bin/sdl2-config.${ABI} || die
	fi
}
