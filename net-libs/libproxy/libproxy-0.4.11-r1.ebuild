...
# $Header: /var/cvsroot/gentoo-x86/net-libs/libproxy/libproxy-0.4.11-r1.ebuild,v 1.7 2013/11/30 20:12:00 pacho Exp $
...
RDEPEND="gnome? ( >=dev-libs/glib-2.26:2[${MULTILIB_USEDEP}] )
	kde? ( >=kde-base/kdelibs-4.4.5[${MULTILIB_USEDEP}] )
	mono? ( dev-lang/mono )
...
src_prepare() {
	...
	# Allow the use of -DDISABLE_BINDINGS to prevent langauage deps on
	# non-native ABI.  If a specific binding is necessary for a non-native ABI
	# package this will need to be re-evalued.
	epatch "${FILESDIR}/${PN}-0.4.11-disable-bindings.patch"
}

libproxy_multilib_configure() {
	...
	if multilib_build_binaries ; then
		...
		mycmakeargs+=(
			...
			-DWITH_PERL=OFF
			-DWITH_PYTHON=OFF
			-DDISABLE_BINDINGS=ON
		)
	fi

	multilib_is_native_abi && use python && python_export_best
	cmake-utils_src_configure
}

src_configure() {
	# cmake-multilib_src_configure does not support seperate
	# cmakeargs for each ABI so setup env and call cmake-utils_src_configure
	# for each ABI instead.
	multilib_parallel_foreach_abi libproxy_multilib_configure
}
