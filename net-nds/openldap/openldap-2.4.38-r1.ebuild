...
inherit db-use eutils flag-o-matic multilib ssl-cert versionator \
		toolchain-funcs autotools user systemd multilib-minimal
...
RDEPEND="icu? ( dev-libs/icu[${MULTILIB_USEDEP}] )
	ssl? ( !gnutls? ( dev-libs/openssl[${MULTILIB_USEDEP}] )
		gnutls? ( net-libs/gnutls[${MULTILIB_USEDEP}] dev-libs/libgcrypt[${MULTILIB_USEDEP}] ) )
	sasl? ( dev-libs/cyrus-sasl[${MULTILIB_USEDEP}] )
	!minimal? (
		sys-devel/libtool[${MULTILIB_USEDEP}]
		tcpd? ( sys-apps/tcp-wrappers[${MULTILIB_USEDEP}] )
		odbc? ( !iodbc? ( dev-db/unixODBC[${MULTILIB_USEDEP}] )
			iodbc? ( dev-db/libiodbc[${MULTILIB_USEDEP}] ) )
		slp? ( net-libs/openslp[${MULTILIB_USEDEP}] )
		perl? ( dev-lang/perl[-build(-)] )
		samba? ( dev-libs/openssl[${MULTILIB_USEDEP}] )
		berkdb? ( sys-libs/db[${MULTILIB_USEDEP}] )
		smbkrb5passwd? (
			dev-libs/openssl
			app-crypt/heimdal )
		kerberos? ( virtual/krb5 )
		cxx? ( dev-libs/cyrus-sasl[${MULTILIB_USEDEP}] )
	)
...
build_contrib_module() {
	lt="${BUILD_DIR}/libtool"
	# <dir> <sources> <outputname>
	mkdir -p "${BUILD_DIR}/contrib/slapd-modules/$1"
	cd "${BUILD_DIR}/contrib/slapd-modules/$1"
	...
	"${lt}" --mode=compile --tag=CC \
		...
		-I${S}/include -I${BUILD_DIR}/include -I${S}/servers/slapd ${CFLAGS} \
		-o ${2%.c}.lo -c ${S}/contrib/slapd-modules/$1/$2 || die "compiling $3 failed"
...
src_configure_cxx() {
	...
	if ! use minimal ; then
		 if use cxx ; then
			...
			myconf_ldapcpp="${myconf_ldapcpp} --with-ldap-includes=${BUILD_DIR}/include"
			mkdir -p "${BUILD_DIR}/contrib/ldapc++"
			cd "${BUILD_DIR}/contrib/ldapc++"
			OLD_LDFLAGS="$LDFLAGS"
...
multilib_src_compile() {
	...
	if ! use minimal && multilib_is_native_abi ; then
		...
		mkdir -p "${BUILD_DIR}/contrib/slapd-modules"

		if use smbkrb5passwd ; then
			einfo "Building contrib-module: smbk5pwd"
			cp -al "${S}/contrib/slapd-modules/smbk5pwd" \
				"${BUILD_DIR}/contrib/slapd-modules/"
			cd "${BUILD_DIR}/contrib/slapd-modules/smbk5pwd"
		...
		if use overlays ; then
			...
			cp -al "${S}/contrib/slapd-modules/samba4" \
				"${BUILD_DIR}/contrib/slapd-modules/"
			cd "${BUILD_DIR}/contrib/slapd-modules/samba4"
		...
		if use kerberos ; then
			mkdir -p "${BUILD_DIR}/contrib/slapd-modules/passwd"
			cd "${BUILD_DIR}/contrib/slapd-modules/passwd"
			...
			"${lt}" --mode=compile --tag=CC \
				...
				-DHAVE_KRB5 \
				-o kerberos.lo \
				-c ${S}/contrib/slapd-modules/passwd/kerberos.c || die "compiling pw-kerberos failed"
		...
		"${lt}" --mode=compile --tag=CC \
			...
			${CFLAGS} \
			-o netscape.lo \
			-c ${S}/contrib/slapd-modules/passwd/netscape.c || die "compiling pw-netscape failed"
		...
		# build slapi-plugins
		mkdir -p "${BUILD_DIR}/contrib/slapi-plugins/addrdnvalues"
		cd "${BUILD_DIR}/contrib/slapi-plugins/addrdnvalues"
		...
		${CC} -shared \
			...
			${S}/contrib/slapi-plugins/addrdnvalues/addrdnvalues.c \
				|| die "Building libaddrdnvalues-plugin.so failed"
...
multilib_src_install() {
	...
	if ! use minimal; then
		...
		if multilib_is_native_abi ; then
			...
			cd "${BUILD_DIR}/contrib/slapd-modules"
			for l in */*.la; do
			...
			cd "${BUILD_DIR}/contrib/slapi-plugins"
...
openldap_multilib_preinst() {
	# keep old libs if any
	preserve_old_lib /usr/$(get_libdir)/{liblber,libldap_r,liblber}-2.3$(get_libname 0)
}

pkg_preinst() {
	multilib_parallel_foreach_abi openldap_multilib_preinst
}

openldap_multilib_postinst() {
...
pkg_postinst() {
	...
	multilib_parallel_foreach_abi openldap_multilib_postinst
}
