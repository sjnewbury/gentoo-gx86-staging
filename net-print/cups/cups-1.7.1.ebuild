...
src_prepare() {
	...
	# *REALLY* doesn't support out-of-source builds!
	multilib_copy_sources
}

post_configure_fixup() {
	cd "${BUILD_DIR}"
...
multilib_src_configure() {
	...
	if multilib_is_native_abi ; then
		...
		# Disable options which do not affect CUPS libraries but pull in
		# extra dependencies
		myeconfargs+=(
			--disable-pam
			--disable-libusb
...
multilib_src_install_all() {
	...
	systemd_newtmpfilesd "${FILESDIR}"/cups.tmpfilesd cups.conf

	# the following files are now provided by cups-filters:
	rm -r "${ED}"/usr/share/cups/banners || die
	rm -r "${ED}"/usr/share/cups/data/testprint || die

	# the following are created by the init script or tmpfiles.d
