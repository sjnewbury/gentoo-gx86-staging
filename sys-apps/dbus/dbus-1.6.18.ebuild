...
inherit autotools eutils linux-info flag-o-matic python-any-r1 systemd virtualx user multilib-minimal
...
multilib_src_configure() {
	...
	if use test; then
		...
		cd "${BUILD_DIR}-test"
		einfo "Running configure in ${BUILD_DIR}-test"
		ECONF_SOURCE="${S}" econf "${myconf[@]}" \
...
multilib_src_compile() {
	...
	if use test; then
		cd "${BUILD_DIR}-test"
		einfo "Running make in ${BUILD_DIR}-test"
		emake
	fi
}
