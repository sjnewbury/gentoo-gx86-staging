...
src_prepare() {
	...
	epatch "${FILESDIR}"/1.5.20/${PN}-1.5.20-x32.patch
	cd ..
...
libtool_multilib_preinst() {
	preserve_old_lib /usr/$(get_libdir)/libltdl.so.3
}

pkg_preinst() {
	multilib_parallel_foreach_abi preserve_old_lib libtool_multilib_preinst
}

libtool_multilib_postinst() {
	preserve_old_lib_notify /usr/$(get_libdir)/libltdl.so.3
}

pkg_postinst() {
	multilib_parallel_foreach_abi libtool_multilib_postinst
}
