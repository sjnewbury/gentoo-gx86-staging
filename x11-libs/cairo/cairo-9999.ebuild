...
SLOT="0"
IUSE="X aqua debug directfb drm gallium gles2 +glib legacy-drivers lto opengl openvg qt4 static-libs +svg valgrind xcb xlib-xcb glx"
...
RDEPEND="dev-libs/lzo[${MULTILIB_USEDEP}]
	...
	X? (
		...
		drm? (
			>=virtual/libudev-136[${MULTILIB_USEDEP}]
...
src_prepare() {
	...
	epatch "${FILESDIR}"/0001-Add-cairo_egl_device_create_for_egl_surface.patch
	#epatch "${FILESDIR}"/0002-Manually-transpose-the-matrix-in-_cairo_gl_shader_bi.patch
...
multilib_src_configure() {
	...
	if ( use gles2 || use opengl || use openvg ); then
		myopts+=" --enable-egl"
	fi
