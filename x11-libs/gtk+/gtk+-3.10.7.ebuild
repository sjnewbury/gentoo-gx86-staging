...
src_prepare() {
	...
	if use test; then
		...
		#	-i testsuite/gtk/testing.c || die "sed 1 failed"
		sed '\%/recent-manager/add%,/recent_manager_purge/ d' \
			-i testsuite/gtk/recentmanager.c || die "sed 2 failed"
		...
		sed -e 's:\(SUBDIRS.*\)reftests:\1:' \
			-i testsuite/Makefile.* || die "sed 3 failed"

		# Test results depend on the list of mounted filesystems!
		rm -f testsuite/a11y/pickers.{ui,txt} || die "rm failed"
		...
		rm -f testsuite/a11y/buttons.{ui,txt} || die
	else
		# don't waste time building tests
		strip_builddir SRC_SUBDIRS testsutie Makefile.am
